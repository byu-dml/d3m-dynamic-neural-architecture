...
class Regression(BaseProblem):
    ...
    def get_correlation_coefficient(self, dataset_performances_map, k=25):
        ...
        for dataset_performance in dataset_performances:
            ...
            best_metric_value_pred = np.nanmax(pandas.DataFrame(dataset_performance[predict_key], dataset_performance[pipeline_key]))
            best_metric_value = np.nanmax(pandas.DataFrame(dataset_performance[actual_key], dataset_performance[pipeline_key]))
