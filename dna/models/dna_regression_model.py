...
class DNARegressionModel(PyTorchRegressionRankSubsetModelBase):
    ...
    def __init__(
        ...
        use_skip: bool = False, dropout = 0.0, aggregate_function: str = "max", *, device: str = 'cuda:0', seed: int = 0
    ):
    ...
    def _get_model(self, train_data):
        ...
        for instance in train_data:
            for step in instance['pipeline']['steps']:
                submodule_input_sizes[step['name']] = len(step['inputs']) if self.aggregate_function == "concat" else 1
        ...
        return DNAModule(
            ...
            self.output_layer_size, self.activation_name, self.use_batch_norm, self.use_skip, self.dropout, self.aggregate_function,
